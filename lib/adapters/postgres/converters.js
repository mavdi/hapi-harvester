...
module.exports = function() {
    ...
    const toSequelizeModel = function (db, hhSchema, models) {
        hhSchema.options = hhSchema.options || {}
        ...
        _.each(hhSchema.attributes, (attribute, key) => {
            ...
            if (hhSchema.options.unique && hhSchema.options.unique.indexOf(key) > -1) {
                 sequelizeSchema[`attributes_${key}`].unique = true
            }
        })

        let schema = db.define(hhSchema.type, sequelizeSchema)

        _.each(hhSchema.relationships, (relationship, key) => {
            let modelType = _.isArray(relationship.data) ? relationship.data[0].type : relationship.data.type 
            Hoek.assert(models[modelType], `model ${modelType} is not yet defined, current limitaitons is that target relationships will need to be defined first`)
            schema.hasOne(models[modelType], { foreignKey: 'initiator_id' })
        })
    ...
    const toSequelizePredicate = function(filter) {
        ...
        return {
            where: _.mapValues(mappedToModel, function (val, key) {
